...
int ku_test() {
  ...
  vm = kut_new(false);
  ku_lexinit(vm, "&& class else false for function if null or return super this true while {}[]!+-*/=!==><>=<= === => break continue const far\ttrick\nart\rcool eek too functiond");
  kutok t = ku_scan(vm);
  ...
  vm = kut_new(true);
  res = ku_exec(vm, "let x=2; let a = (x > 2) && 7 or 6;");
  ...
  vm = kut_new(true);
  res = ku_exec(vm, "let comp = (a, b) => ( (a < b) && -1 or ( (a > b) && 1 or 0)) ;");
